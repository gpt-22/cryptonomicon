...
/* TODO:
наличие в состоянии зависимых данных, критичность: 6 +
API запросы напрямую внутри компонента -> нарушение single responsibility, критичность: 5 +
при тикера удалении остаётся подписка на загрузку данных о нём, критичность: 5 +
количество запросов, критичность: 4 +
несколько вкладок не работают, критичность: 5
обработка ошибок API, критичность: 5
обновить lastPage при закгрузке страницы, критичность: 5 +
localStorage и анонимные вкладки, критичность: 3
вид графика при множестве цен, критичность: 3
одинаковый код в watch, критичность: 2 +
magic numbers and strings, критичность: 1
---------------
При удалении всех coins страница должна быть 1, а не 0 +
При загрузке страницы пофиксить disabled у кнопок вперед и назад +
При добавлении тикера страница всегда переключается на последнюю +
При клике на график больше раза он сбрасывается +
 */
