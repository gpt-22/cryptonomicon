          ...
          <div class="max-w-xs">
            ...
            <div v-if="hints.length" class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span
                  v-for="hint in hints"
                  :key="hint"
                  @click="setTickerInInput(hint)"
                  class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
                {{ hint }}
              </span>
            </div>
            <div v-show="tickerInTickers" class="text-sm text-red-600">Такой тикер уже добавлен</div>
          </div>
        </div>
        <button
            @click="addCoin"
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
              ...
              :key="ticker.Id"
              :class="isActive(ticker) ? 'border-4' : ''"
...
export default {
  ...
  methods: {
    ...
    filterHints() {
      if (this.ticker) {
        ...
        this.hints = keys.filter(key => {
          const coin = this.data[key]
          return coin.Symbol.startsWith(this.ticker) || coin.FullName.startsWith(this.ticker)
        }).slice(0, 4)
      } else this.hints = []
    },
  ...
  computed: {
    tickerInTickers() {
      const coin = this.getCoin()
      return this.tickers.includes(coin)
    }
  }
}
</script>
